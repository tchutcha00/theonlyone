...
function onUse(cid, item, fromPosition, itemEx, toPosition)  
    ...
    if isWeapon(itemEx.uid) then
        if math.random(1,100) < 100 then
            doItemSetAttribute(itemEx.uid, 'attackspeed', 300)
            doSendAnimatedText(getPlayerPosition(cid), "ENCHANTED!", TEXTCOLOR_PURPLE)
            doBroadcastMessage("[MENSAGEM]: O jogador (" .. getCreatureName(cid) .. ") evoluiu sua ".. getItemNameById(itemEx.itemid) .." para speed nivel 4.", 19)
        else
