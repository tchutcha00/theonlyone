...
function onUse(cid, item, fromPosition, itemEx, toPosition)  
    ...
    if isWeapon(itemEx.uid) then
        if math.random(1,100) < 100 then
            ...
            doSendAnimatedText(getPlayerPosition(cid), "ENCHANTED!", TEXTCOLOR_DARKYELLOW)
            doBroadcastMessage("[MENSAGEM]: O jogador (" .. getCreatureName(cid) .. ") evoluiu sua ".. getItemNameById(itemEx.itemid) .." para speed nivel 5.", 19)
        else
