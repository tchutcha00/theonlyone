...
jailpos = { x = 1036, y = 1094, z =7 }
unjailpos = { x = 1095, y = 1062, z =7 }
jail_list = {}
jail_list_work = 0
...
doPlayerSendTextMessage ( isplayer, MESSAGE_INFO_DESCR, 'você fez besteira e foi preso, por '.. getCreatureName(cid) ..' até ' .. os.date("%H:%M:%S", getPlayerStorageValue(isplayer, jailedstoragevalue_time)) .. ' (agora são: ' .. os.date("%H:%M:%S", os.time()) .. ').')
return TRUE
else
doPlayerSendTextMessage(cid, MESSAGE_INFO_DESCR, "O jogador com esse nome nao existe ou esta offline.")
return FALSE
end
else
doPlayerSendTextMessage(cid, MESSAGE_INFO_DESCR, "voce nao pode prender outros players.")
return FALSE
end
