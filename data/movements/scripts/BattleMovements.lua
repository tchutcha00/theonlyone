...
doPlayerSendTextMessage(cid, 19, "Você entrou no time " .. _Lib_Battle_Info.TeamOne.name .. "!")
else
...
doPlayerSendTextMessage(cid, 19, "Você entrou no time " .. _Lib_Battle_Info.TeamTwo.name .. "!")
end
end
if getGlobalStorageValue(_Lib_Battle_Info.storage_count) == 0 then
removeBattleTp()
doBroadcastMessage("[Evento Battlefield] vai começar em 1 minuto, porfavor crie sua estratégia!", MESSAGE_STATUS_CONSOLE_BLUE)
addEvent(doBroadcastMessage, 1*60*1000-500, "[Evento Battlefield] começou agora!", MESSAGE_STATUS_CONSOLE_BLUE)
