...
function onCreatureSay(cid, type, msg)
   ...
   elseif (msgcontains(msg, "hi") and (focus == 0)) and getDistanceToCreature(cid) < 4 then
      ...
      if (_CTF_LIB.nopen) and (getGlobalStorageValue(_CTF_LIB.CTFSto) < 1) then
         selfSay("Olá {" .. getCreatureName(cid) .. "}, sou responsável por levar os players para o evento Capture The Flag, porém ele está fechado no momento.", cid)
      else
         selfSay("Olá {" .. getCreatureName(cid) .. "}, sou responsável por levar os players para o evento Capture The Flag, caso esteja interessado em ir diga {ctf}.", cid)
         talkState[talkUser] = 0                                 
      end            
   elseif (msgcontains(msg, "ctf")) and (talkState[talkUser] == 0) and (focus == cid) and (getDistanceToCreature(cid) < 4) then
