...
local config = {
   ...
   item = 12629,
   quant = 1,
   storagesorte = 4003 -- storage que será verificado
   }


if(msgcontains(msg, 'lucky' )) then
npcHandler:say("Voce deseja evoluir seu nível de Lucky?", cid)
...
elseif msgcontains(msg, {"yes","sim"}) then
	if getPlayerStorageValue(cid, config.storagesorte) < config.levelsorte then
		if getPlayerItemCount(cid, config.item) >= config.quant then
						...
						doPlayerSendTextMessage(cid,19,"Voce evoluiu lucky para "..(getPlayerStorageValue(cid, config.storagesorte)+1).." parabéns, verifique em seu !serverinfo.")
			...
			selfSay('Você não tem o item necessario {crystal of lucky}.', cid)
		end
	else
